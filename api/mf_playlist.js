...
export default async function handler(req, res) {
  try {
    // повертаємо кеш, якщо не прострочено
    ...
    for (const link of [...new Set(matchLinks)]) {
      ...
      // ловимо будь-який запит на .m3u8
      ...
      try {
        await matchPage.goto(link, { waitUntil: "networkidle2", timeout: 60000 });

        // даємо плеєру завантажитися
        ...
        if (foundStream) {
          const title = link.split("/").pop()
            .replace(".html", "")
            .replace(/-/g, " ");
          streams.push({ title, url: foundStream });
        ...
        console.log("Error opening match:", err.message);
      }
